...
class AIOParse:
    ...
    async def parse(self, page: Union[str, List[str], Future]) -> Union[List[str], str, List[Future]]:
        ...
        if isinstance(page, str):
            ...
            return pages
        elif isinstance(page, Future):
            await self.logger.debug(f"[PARSER] START FUTURE")
            pages = []
            for request_future in asyncio.as_completed([self.func_runner(await i) for i in page]):
                pages.append(request_future)
            return pages
        else:
